  ...
  <div class="min-h-screen bg-gray-50 dark:bg-gray-950 flex">
    ...
    >
      ...
      <nav class="flex-1 overflow-y-auto py-4 space-y-0.5" :class="collapsed ? 'px-2' : 'px-3'">
        ...
        <NavLink :href="route('empresa.prestadores.index')" :active="isActive('/empresa/prestadores')"
          :collapsed="collapsed" label="Buscar Prestadores" active-color="teal">
          <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const showStatusBadge = computed(() => {
  const c = page.props.auth.company
  if (!c) return false
  if (c.status !== 'approved') return true
  if (!c.approved_at) return true
  return (Date.now() - new Date(c.approved_at).getTime()) < THIRTY_DAYS_MS
})

const statusClass = computed(() => ({
  'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300': page.props.auth.company?.status === 'pending',
  'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300':     page.props.auth.company?.status === 'approved',
  'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300':             page.props.auth.company?.status === 'rejected',
}))
