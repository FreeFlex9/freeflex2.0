  ...
  <div class="min-h-screen bg-gray-50 dark:bg-gray-950 flex">
    ...
    >
      ...
      <nav class="flex-1 overflow-y-auto py-4 space-y-0.5" :class="collapsed ? 'px-2' : 'px-3'">
        ...
        <NavLink :href="route('prestador.mensagens.index')" :active="isActive('/prestador/mensagens')"
          :collapsed="collapsed" label="Mensagens" active-color="orange" :badge="chatUnread">
          <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <NavLink :href="route('prestador.ponto.index')" :active="isActive('/prestador/ponto')"
          :collapsed="collapsed" label="Check-in/Check-out" active-color="orange">
          <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
onMounted(() => {
  const providerId = page.props.auth.provider?.id
  if (!providerId || !window.Echo) return
  inboxChannel = window.Echo.private(`chat-inbox.provider.${providerId}`)
    .listen('.chat.inbox.ping', (e) => { chatUnread.value = e.unread_count })
})

onUnmounted(() => {
  const providerId = page.props.auth.provider?.id
  if (inboxChannel && providerId) window.Echo?.leave(`chat-inbox.provider.${providerId}`)
})
