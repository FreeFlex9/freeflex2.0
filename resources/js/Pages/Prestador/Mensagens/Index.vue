<template>
  <PrestadorLayout title="Mensagens">
      ...
      <div class="flex h-full">
        ...
        <div class="w-full lg:w-80 shrink-0 border-r border-gray-100 dark:border-gray-800 flex flex-col"
          ...
          <div class="flex-1 overflow-y-auto">
            <p v-if="conversas.length === 0" class="text-center text-gray-400 text-sm py-8 px-4">
              Nenhuma conversa ainda. Envie uma proposta para começar.
            </p>
            <button v-for="c in conversas" :key="c.proposal_id" @click="selecionar(c)"
              ...
              :class="selecionada?.proposal_id === c.proposal_id && 'bg-orange-50 dark:bg-orange-500/10'">
              ...
              <p class="text-xs text-gray-400 truncate mt-0.5">{{ c.company_name ?? 'Empresa' }}</p>
        ...
        <div class="flex-1 min-w-0" :class="selecionada ? 'flex' : 'hidden lg:flex'">
          <ChatPanel v-if="selecionada" ref="panel"
            ...
            :disabled="thread.sending.value" current-role="provider"
            :title="selecionada.demand_title ?? 'Demanda'" :subtitle="selecionada.company_name"
            :other-label="selecionada.company_name ?? 'Empresa'"
...
function selecionar(c) {
  ...
  thread = useChatThread({
    fetchUrl:    route('prestador.propostas.mensagens', c.proposal_id),
    sendUrl:     route('prestador.propostas.mensagens.enviar', c.proposal_id),
    channelName: `chat.${c.demand_id}.provider`,
  })
