        ...
        >
          <div class="flex flex-wrap items-start justify-between gap-4">
            <div class="flex-1 min-w-0">
              <h3 class="font-semibold text-gray-800 text-lg truncate">{{ emp.nome_fantasia }}</h3>
              <div class="mt-1 space-y-0.5 text-sm text-gray-500">
                ...
                <p v-if="emp.telefone">Telefone: {{ emp.telefone }}</p>
                <p>Cadastro: {{ formatDate(emp.data_cadastro) }}</p>
              </div>
            </div>
            ...
            <div class="flex flex-col gap-2 items-end">
              <!-- Cartão CNPJ -->
              <a
                v-if="emp.cartao_cnpj_path"
                :href="'/' + emp.cartao_cnpj_path"
                target="_blank"
              ...
              <div class="flex gap-2 mt-2">
                <!-- Aprovar -->
                <form @submit.prevent="aprovar(emp)" :class="{ 'opacity-50': !emp.cartao_cnpj_path }">
                  <button
                    type="submit"
                    :disabled="!emp.cartao_cnpj_path || loading === emp.id"
                    class="px-4 py-2 bg-green-500 text-white text-sm font-medium rounded-lg hover:bg-green-600 disabled:cursor-not-allowed transition-colors"
                  >
                    Aprovar
                  </button>
                </form>
    ...
    <div v-if="modalEmpresa" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl w-full max-w-md p-6">
        <h3 class="text-lg font-semibold text-gray-800 mb-4">
          Rejeitar: {{ modalEmpresa.nome_fantasia }}
        </h3>
        <textarea
          v-model="motivo"
          rows="4"
        ...
        <div class="flex gap-2 mt-4">
          <button @click="fecharModal" class="flex-1 px-4 py-2 border border-gray-300 text-gray-700 rounded-lg text-sm hover:bg-gray-50">
            Cancelar
          </button>
          <button @click="rejeitar" class="flex-1 px-4 py-2 bg-red-500 text-white rounded-lg text-sm font-medium hover:bg-red-600">
            Confirmar Rejeição
          </button>
        </div>
      </div>
...
function aprovar(emp) {
  if (!confirm(`Aprovar a empresa "${emp.nome_fantasia}"?`)) return
  loading.value = emp.id
  useForm({}).post(route('admin.empresas.aprovar', emp.id), {
    onFinish: () => { loading.value = null },
  })
}

function abrirRejeicao(emp) {
  modalEmpresa.value = emp
  motivo.value = ''
  erroMotivo.value = ''
}

function fecharModal() {
  modalEmpresa.value = null
}

function rejeitar() {
  if (!motivo.value.trim()) {
    erroMotivo.value = 'O motivo é obrigatório.'
    return
  }
